...
const fs = require('fs');
// let rawdata = fs.readFileSync('xian.json');
...
router.get('/', (req, res) => {
  // student.flat()
  res.json({msg:0})
  // getData([{"adcode":100000,"name":"全国"}]).then(value=>{
  // })
})
...
const getList=async (item)=>{
  ...
  try {
    await write(`./geo/${item.adcode}.json`, JSON.stringify(value))
    console.log(item.name+"写入成功")
  } catch (error) {
    console.log(item.name+"写入失败")
  }
  return "ok"
  // return value?value.features.map(item=>{
